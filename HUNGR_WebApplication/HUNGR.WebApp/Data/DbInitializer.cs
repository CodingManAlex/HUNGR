...
namespace Week2_IdentitySystem.Data
{
    public static class DbInitializer
    {
        ...
        public static async Task<int> SeedUsersAndRoles(IServiceProvider serviceProvider)
        {
            ...
            var context = serviceProvider.GetRequiredService<HUNGRDbContext>();
        ...
        private static async Task<int> SeedRoles(RoleManager<IdentityRole> roleManager)
        {
            ...
            result = await roleManager.CreateAsync(new IdentityRole("FoodTruck"));
            if (!result.Succeeded)
                return 3;  // should log an error message here
        ...
        private static async Task<int> SeedUsers(UserManager<ApplicationUser> userManager)
        {
            ...
            {
                UserName = "seededuser@hotmail.com",
                Email = "seededuser@hotmail.com",
                FirstName = "Seeded",
                LastName = "User",
                City = "Hamilton",
                Province = "Ontario",
            ...
            result = await userManager.CreateAsync(testUser, appSecrets.MemberPwd);
            if (!result.Succeeded)
                return 3;  // should log an error message here

            // Assign user to PLayer role
            result = await userManager.AddToRoleAsync(testUser, "User");
            if (!result.Succeeded)
                return 4;  // should log an error message here
